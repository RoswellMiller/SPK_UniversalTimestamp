    ...
    def test_appendix_c_Construction(self):
        ...
        for appendix_c_ndx in range(len(self.appendix_c_table['R.D.'])):
            ...
            if test_weekday_ndx is None:
                ...
                failures += 1
                continue      
            test_day = self.appendix_c_table[f'{calendar}'][appendix_c_ndx]
            msg = f"Row {appendix_c_ndx+1} {calendar} day number: {test_day}"
            try:
                moment = UnivMoment.from_julian_day_number(test_day)
