"""
Comprehensive tests for the Moment_Hebrew class.
"""
import json
import os

from SPK_UniversalTimestamp.Constants_aCommon import Calendar
from SPK_UniversalTimestamp.Constants_Hebrew import hebrew_MONTH_ATTS
...
class Test_Moment_Hebrew: 
    """Test cases for Moment_Hebrew class."""
    ...
    def test_appendix_c_Construction(self):
        ...
        for appendix_c_ndx in range(len(self.appendix_c_table['R.D.'])):
            ...
            try:
                moment = UnivMoment.from_hebrew(test_year, test_month, test_day, precision=UnivMomPrecision.DAY)
    ...
    def test_appendix_c_Presentation(self):
        ...
        for appendix_c_ndx in range(len(self.appendix_c_table['R.D.'])):
            ...
            {   'format': "%d %b. %Y",  'condition' : lambda : True,
                'answer': f"{test_day:02d} {hebrew_MONTH_ATTS['en'][test_month]['abbrv']}. {test_year:d}"},
            {   'format': "%A %d %B, %Y",  'condition' : lambda : True,
                'answer': f"{test_weekday} {test_day:02d} {hebrew_MONTH_ATTS['en'][test_month]['name']}, {test_year:d}"},
            {   'format': "%A %d %B, %y", 'condition' : lambda : test_year >= 0,
                'answer': f"{test_weekday} {test_day:02d} {hebrew_MONTH_ATTS['en'][test_month]['name']}, {test_year:d}"},
            {   'format': "%a %d %B, %y", 'condition' : lambda : test_year < 0,
                'answer': f"{Present_Calendars.DAY_OF_THE_WEEK_ATTS['en'][test_weekday_ndx]['abbrv']} {test_day:02d} {hebrew_MONTH_ATTS['en'][test_month]['name']}, {abs(test_year):d} bc"},
            ]
            ...
            try:
                for test_case in test_cases:
                    ...
                    presentation = moment.present(Calendar.HEBREW, test_case['format'], language='en')
                    if presentation == test_case['answer']:
                        print(f"    ✅ {msg}")
                    else:
                        print(f"    ❌ MATCH R.D.Error {test_rd}")
