...
class TestUniversalTimestamp:
    ...
    def test_sorting_and_comparison(self):
        ...
        # Test sorting
        timestamps = [modern_2, future, big_bang, modern, dinosaurs, bce]
        timestamps.sort(key=lambda ts: ts.sort)
        
        # Should be: big_bang, dinosaurs, modern, future
        assert timestamps[0] == big_bang
        assert timestamps[1] == dinosaurs
        assert timestamps[2] == bce
        assert timestamps[3] == modern
        assert timestamps[4] == modern_2
        assert timestamps[5] == future
